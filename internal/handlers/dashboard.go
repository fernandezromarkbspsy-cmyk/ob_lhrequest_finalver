...
func LoginAPI(c echo.Context) error {
	...
	if err := verifyPasswordPolicy(user, payload.Password); err != nil {
		return err
	}
...
func verifyPasswordPolicy(user models.User, password string) error {
	password = strings.TrimSpace(password)
	requirePassword := strings.EqualFold(os.Getenv("AUTH_REQUIRE_PASSWORD"), "true")
	if user.PasswordHash == "" {
		if requirePassword {
			return echo.NewHTTPError(http.StatusUnauthorized, "Password is required for this account")
		}
		return nil
	}
	if password == "" {
		return echo.NewHTTPError(http.StatusUnauthorized, "Password is required")
	}
...
func RequestsAPI(c echo.Context) error {
	page := parsePositiveInt(c.QueryParam("page"), 1, 100000)
	pageSize := parsePositiveInt(c.QueryParam("page_size"), 50, 250)
	rows, total, err := queryRequestRows(c.QueryParam("queue"), c.QueryParam("status"), c.QueryParam("search"), c.QueryParam("date_from"), c.QueryParam("date_to"), page, pageSize)
	if err != nil {
...
func EditRequestAPI(c echo.Context) error {
	...
	return updateRequest(c, "edit", func(request *models.Request, payload requestPayload, now time.Time) error {
...
func CancelRequestAPI(c echo.Context) error {
	return updateRequest(c, "cancel", func(request *models.Request, payload requestPayload, now time.Time) error {
		remarks := strings.TrimSpace(payload.Remarks)
		if remarks == "" {
			return echo.NewHTTPError(http.StatusBadRequest, "Remarks are required to cancel")
		}
...
func ApproveRequestAPI(c echo.Context) error {
	return updateRequest(c, "approve", func(request *models.Request, payload requestPayload, now time.Time) error {
		obfte := strings.TrimSpace(payload.OBFTE)
		if obfte == "" {
			return echo.NewHTTPError(http.StatusBadRequest, "OB FTE is required to approve")
		}
...
func RejectRequestAPI(c echo.Context) error {
	return updateRequest(c, "reject", func(request *models.Request, payload requestPayload, now time.Time) error {
		remarks := strings.TrimSpace(payload.Remarks)
		if remarks == "" {
			return echo.NewHTTPError(http.StatusBadRequest, "Remarks are required to reject")
		}
...
func AssignRequestAPI(c echo.Context) error {
	return updateRequest(c, "assign", func(request *models.Request, payload requestPayload, now time.Time) error {
		midmileFTE := strings.TrimSpace(payload.MidmileFTE)
		if midmileFTE == "" {
			return echo.NewHTTPError(http.StatusBadRequest, "Midmile FTE is required to assign")
		}
		request.Status = StatusAssigned
		request.MidmileFTE = midmileFTE
...
func ForDockingRequestAPI(c echo.Context) error {
	return updateRequest(c, "for-docking", func(request *models.Request, payload requestPayload, now time.Time) error {
		midmileFTE := strings.TrimSpace(payload.MidmileFTE)
		plateNumber := strings.TrimSpace(payload.PlateNumber)
		if midmileFTE == "" || plateNumber == "" {
			return echo.NewHTTPError(http.StatusBadRequest, "Midmile FTE and plate number are required for docking")
		}
...
func DockRequestAPI(c echo.Context) error {
	return updateRequest(c, "dock", func(request *models.Request, payload requestPayload, now time.Time) error {
		driverID := strings.TrimSpace(payload.DriverID)
		tripNo := strings.TrimSpace(payload.LinehaulTripNo)
		if driverID == "" || tripNo == "" || strings.TrimSpace(payload.DockingTime) == "" {
			return echo.NewHTTPError(http.StatusBadRequest, "Driver ID, trip number, and docking time are required")
		}
...
func ConfirmRequestAPI(c echo.Context) error {
	return updateRequest(c, "confirm", func(request *models.Request, payload requestPayload, now time.Time) error {
		if request.DriverID == "" || request.LinehaulTripNo == "" || request.DockedTime == nil {
			return echo.NewHTTPError(http.StatusBadRequest, "Request must be docked with driver, trip number, and docking time before confirmation")
		}
...
func CreateUserAPI(c echo.Context) error {
	...
	passwordHash, err := passwordHashFromPayload(payload.Password)
	if err != nil {
		return err
	}
...
func updateRequest(c echo.Context, action string, mutate func(*models.Request, requestPayload, time.Time) error) error {
	if database.DB == nil {
		return echo.NewHTTPError(http.StatusServiceUnavailable, "Database is not configured")
	}
	...
	err = database.DB.Transaction(func(tx *gorm.DB) error {
		...
		if !isAllowedTransition(action, previousStatus) {
			return echo.NewHTTPError(http.StatusConflict, "Request cannot be "+action+" in its current status")
		}

		if err := mutate(&request, payload, time.Now()); err != nil {
			return err
		}
		request.Status = normalizeStatus(request)
	...
	if normalizeStatus(updated) != previousStatus {
		eventType = events.RequestStatusChanged
	}
	publishRequestEvent(eventType, action, previousStatus, updated)

	return c.JSON(http.StatusOK, requestToRow(updated))
}

func isAllowedTransition(action, status string) bool {
	allowed := map[string][]string{
		"edit":        {StatusPendingOps, StatusRejected},
		"approve":     {StatusPendingOps},
		"reject":      {StatusPendingOps},
		"assign":      {StatusPendingMM},
		"for-docking": {StatusAssigned},
		"dock":        {StatusForDocking},
		"confirm":     {StatusDocked},
		"cancel":      {StatusPendingOps, StatusPendingMM, StatusAssigned},
	}
	for _, allowedStatus := range allowed[action] {
...
func passwordHashFromPayload(password string) (string, error) {
	password = strings.TrimSpace(password)
	if password == "" {
		if strings.EqualFold(os.Getenv("AUTH_REQUIRE_PASSWORD"), "true") {
	...
	if err != nil {
		return "", echo.NewHTTPError(http.StatusInternalServerError, "Unable to secure password")
	}
...
func loadRequestRows(queue string) []RequestRow {
	rows, _, _ := queryRequestRows(queue, "", "", "", "", 1, 250)
	return rows
}
...
func queryRequestRows(queue, status, search, dateFrom, dateTo string, page, pageSize int) ([]RequestRow, int64, error) {
	if database.DB == nil {
	...
	if err := query.Order("request_timestamp desc").Limit(pageSize).Offset(offset).Find(&requests).Error; err != nil {
		return nil, 0, err
	}
